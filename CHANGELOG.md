...
* refactor: Add PHPStan level 8 static analysis (with real type declarations rather than a baseline/ignore list) covering every PHP file in the plugin; added `composer.json`/`phpstan.neon`/`phpstan-stubs.php` (Cacti core function stubs used only for static analysis, never loaded at runtime) and a Cacti-style `.php-cs-fixer.php`
