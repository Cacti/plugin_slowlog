...
function form_save(): void {
	...
	if (isset($_POST['save_component_import'])) {
		...
		if ($upload_error === UPLOAD_ERR_OK && $_FILES['import_file']['tmp_name'] != '') {
			...
			if (!in_array($table_mode, ['cacti', 'reference', 'all'], true)) {
				$table_mode = 'all';
			}
...
function form_actions(): void {
	...
	if (!array_key_exists($drp_action, $actions)) {
		header('Location: slowlog.php');
		exit;
	}
	...
	html_start_box($actions[$drp_action], '60%', false, 3, 'center', '');
	...
	print "	<tr>
		<td class='saveRow'>
			...
			<input type='hidden' name='drp_action' value='" . html_escape($drp_action) . "'>" . (strlen($save_html) ? "
...
function slowlog_import(): void {
	...
	html_start_box(__('Import MariaDB/MySQL Slowlog', 'slowlog'), '100%', false, 3, 'center', '');
...
function slowlog_view_details(): void {
	...
	html_start_box(__('MariaDB/MySQL SlowLog Details', 'slowlog'), '100%', false, 3, 'center', '');
...
function slowlog_view_charts(string $method): void {
	...
	html_start_box(__('Chart Filters', 'slowlog'), '100%', false, 3, 'center', '');

	slowlog_charts_filter($method, $id);

	html_end_box();

	html_start_box(__('MariaDB/MySQL SlowLog Results - By %s', ucfirst($method), 'slowlog'), '100%', false, 3, 'center', '');
...
function slowlog_request_charts_validation(string $chart_type): void {
	$filters = [
		...
		'chart_top' => [
			...
			'pageset' => true,
			'default' => '10',
			'options' => ['options' => 'slowlog_sanitize_chart_top']
		],
	];
...
function slowlog_view_query(): void {
	...
	$entry = db_fetch_row_prepared('SELECT *
		...
		WHERE logid = ?
		AND logentry = ?',
		[get_filter_request_var('logid'), get_filter_request_var('logentry')]);
	$entry = is_array($entry) ? $entry : [];

	slowlog_tabs();

	html_start_box(__('MariaDB/MySQL SlowLog Query Details', 'slowlog'), '100%', false, 3, 'center', '');
	...
	print '<td>' . __('Query Time', 'slowlog') . '</td><td>' . number_format_i18n($entry['query_time'] ?? 0) . '</td>';
	print '<td>' . __('Lock Time', 'slowlog') . '</td><td>' . number_format_i18n($entry['lock_time'] ?? 0) . '</td>';
	print '<td>' . __('Rows Sent', 'slowlog') . '</td><td>' . number_format_i18n($entry['rows_sent'] ?? 0) . '</td>';
	print '<td>' . __('Rows Examined', 'slowlog') . '</td><td>' . number_format_i18n($entry['rows_examined'] ?? 0) . '</td>';

	form_end_row();

	form_alternate_row();

	print '<td>' . __('Rows Affected', 'slowlog') . '</td><td>' . number_format_i18n($entry['rows_affected'] ?? 0) . '</td>';
	print '<td>' . __('Bytes Sent', 'slowlog') . '</td><td>' . number_format_i18n($entry['bytes_sent'] ?? 0) . '</td>';
	print '<td colspan="4"></td>';

	form_end_row();
	html_end_box(false);

	html_start_box(__('Original Query', 'slowlog'), '100%', false, 3, 'center', '');
...
function slowlog_view(): void {
	...
	html_start_box(__('MariaDB/MySQL SlowLog File Filters', 'slowlog'), '100%', false, 3, 'center', 'slowlog.php?action=edit');
	filter();
	html_end_box();
