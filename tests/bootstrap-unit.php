 ...
 * Test bootstrap.
 *
 * Slowlog's sources expect to be included by Cacti, which has already
...
function slowlog_test_db_result($fn, $sql, $params, $default) {
	...
	if (!empty($GLOBALS['__test_db_fixtures'][$fn])) {
		// Most-recently-registered fixture wins, so a test can override a beforeEach() default.
 ...
 * @return void
 */
function slowlog_test_load($path) {
	global $config;
